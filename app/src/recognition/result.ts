...
export function readRecognition(raw: any): RecognitionResult {
  ...
  const status: RecognitionResult['status'] = raw.status === 'ready' && raw.doubts.length ? 'partial' : raw.status
  ...
  const total = lines.reduce((sum: number, line: UsageLine) => sum + line.qty, 0)
  if (!Number.isSafeInteger(total) || (raw.total != null && (!Number.isSafeInteger(raw.total) || raw.total !== total))) throw Error('候选合计无效')
  if (raw.status === 'failed' && (lines.length || raw.source !== null) || raw.status !== 'failed' && (!lines.length || !['legend', 'grid'].includes(raw.source))) throw Error('候选状态不一致')
  if (raw.titleTotal !== null && (!Number.isSafeInteger(raw.titleTotal) || raw.titleTotal < 0)) throw Error('标题总数无效')
  ...
  if (status !== 'ready') risks.unshift({ id: 'coverage', reason: '完整覆盖尚未证明，仍可能漏计。请结合原图补充用量。', raw: '', region: null, resolved: false })
  if (risks.length > 512) throw Error('识别疑点过多，请换清晰图或手工录入')
  return { algorithm: raw.algorithm, status, source: raw.source, lines, titleTotal: raw.titleTotal, width, height, evidence, risks }
}
export function recognitionProvenance(result: RecognitionResult, manualCorrection = false): RecognitionProvenance {
  if (result.status === 'failed' ? !manualCorrection : !result.source || !result.lines.length) throw Error('失败结果不能作为自动用量')
  return { source: result.source ?? 'assisted', algorithmVersion: result.algorithm, originalStatus: result.status, candidateLines: result.lines.map(l => ({ ...l })), candidateTitleTotal: result.titleTotal, modified: false, risks: result.risks.map(({ region: _, ...risk }) => ({ ...risk })), riskAcknowledged: false }
}
export function hasRecognitionRisk(value?: RecognitionProvenance | null): boolean {
  return !!value && (value.originalStatus !== 'ready' || value.risks.some(r => !r.resolved))
}
/** A correction and its acknowledgement apply only to the current editable quantities. */
export function invalidateRiskReview(value: RecognitionProvenance | null): void {
  if (!value) return
  value.riskAcknowledged = false
  value.risks.forEach(risk => { risk.resolved = false })
}
