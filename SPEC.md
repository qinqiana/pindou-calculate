...
2026-09-23 用户在 `codex/issue-12-body-count` 上继续授权 #13、#14。算法 `pixel-glyph-0.12.0-dev` 按原图行分块、持续保留部分候选和未处理位置，取消/超时/编辑不覆盖手工用量，当前备份格式不变。真实 N25 长图及清晰构造组逐色全对，真实 47×104 水印图本体仍不支持完整自动计数；原图数量表可准确读取。全登记 255 张图纸本轮 155 张逐色逐数一致，99 张色卡外及 1 张截断无完整真值均保留，不称为新盲测。Android 的取消、当前备份恢复、分享、数据保护与性能已记录；独立 Android 容器已完成自签 Release 构建，并在手机完成启动、SQLite 重启保存、媒体输出和真实 N25 识别确认。系统选择器选取、覆盖更新、人工核对成本及用户验收仍待完成，详见 [#13–#14 记录](docs/acceptance/issues13-14/README.md)。用户随后明确同意改用 Android 本身并尽量不引入第三方：独立原生容器使用系统 WebView、SQLite、图片解码和文档/媒体接口，不再依赖 DCloud 手机运行库或 AppKey。现有 Vue/uni-app 页面继续编译为本地网页，账本及备份 v2 规则复用；签名由本机专用密钥完成，qin 保管。实施与验收结果回填同一记录。

2026-09-24 用户确认以 0.2.0 整合分支为基础交付自用版 0.2.1：PNG 数据和 IEND 块完整时允许结束块后的厂商附加信息；截断或损坏的 PNG 仍拒绝，原图文件不作修改。备份格式维持 v2，使用原签名密钥构建可覆盖安装的 Release APK；公开发行仍需另行授权。

2026-09-25 用户改为在 Windows 生成新签名密钥并打包 0.2.1。新证书与 0.2.0 不同，无法覆盖安装旧版；迁移个人账本必须先在旧版导出并核对备份文件，再卸载旧版、安装新包并恢复。后续 0.2.1 更新沿用 Windows 新密钥；备份格式仍为 v2，不授权公开发行。
...
2026-09-24 用户反馈：识别期间应用切到后台不应主动取消当前任务；返回时继续展示仍有效的进度或结果。超过 30 秒未完成时提供继续等待、重试和手工录入的选择；离开编辑会话、换图或数据版本变化仍使旧结果失效。疑点区域及操作按钮使用区别于普通按钮的颜色和文字说明；底部知情确认入口靠近“确认全部用量”，须先引导查看疑点，避免跳过未计入内容。
...
2026-09-22 用户明确允许主会话选择任何能完成 Android 编译与真机测试的工具，不要求 HBuilderX 或 Windows。当前使用 Linux；已通过官方 Node CLI 编译页面资源，并连接 Android 真机。2026-09-23 用户确认改用 Android 系统容器；现已构建系统 WebView/SQLite/文档与媒体接口的独立工程，并生成自签 Release APK。资源、debug 包、Release 包和真机结果分别验证，不混为全部完成。正式签名密钥由 qin 保管；不使用云打包或 DCloud AppKey。[DCloud CLI 与打包说明](https://uniapp.dcloud.net.cn/worktile/CLI.html)、[App 离线 SDK](https://nativesupport.dcloud.net.cn/AppDocs/)
...
| 决策 | 当前结论 | 后续责任 |
| --- | --- | --- |
| D1 架构与构建路径 | Android 系统 WebView 容器与原生 API；保留 Vue 3 / uni-app H5 / TS 页面，Linux 本地构建自签 APK，不用 DCloud 手机运行库或 AppKey；见 [决议](docs/adr/0001-android-platform-container.md) | 主会话实施和验证，不再询问技术栈选择 |
